...
export const FormView = () => {
    ...
    return useObserver(() => {
        return React.createElement(Layout, { style: { height: '100%', overflow: 'hidden' }, className: "fl-form-container" },
            React.createElement(FormHeaderView, { title: store.form.content.title, subtitle: store.form.content.subtitle, desc: store.form.desc, currentPage: store.currentPage.get(), numPages: store.numPages, showSteps: store.form.formLayoutOptions.showSteps }),
            React.createElement(Layout, { style: { overflow: 'hidden', border: '1px solid #ededed' } }, store.isReady.get() && React.createElement(Form, { onSubmit: (e) => store.form.handleSubmit(e), layout: store.form.layout, style: { height: '100%' }, wrapperCol: store.form.formLayoutOptions.wrapperCol, labelCol: store.form.formLayoutOptions.labelCol, labelAlign: store.form.formLayoutOptions.labelAlign },
                React.createElement(Layout.Content, { style: { overflowY: 'scroll', height: 'calc(100% - 68px)', backgroundColor: 'white' } },
                    React.createElement(PageView, { page: store.form.content.pages[store.currentPage.get()] })),
                        ...
                        React.createElement(Card, { bordered: false },
                            React.createElement(Row, null,
                                React.createElement(Col, { span: 24, style: { textAlign: 'right' } },
                                    store.currentPage.get() == store.numPages - 1 && React.createElement(Button, { disabled: Object.keys(store.touched).length == 0 || !store.isValid || store.isSubmitting, type: "primary", style: { marginLeft: 8 }, htmlType: "submit", className: "action-button" }, "Submit"),
                                    store.currentPage.get() < store.numPages - 1 && React.createElement(Button, { type: "primary", style: { marginLeft: 8 }, className: "action-button", onClick: () => store.nextPage() },
                                        React.createElement(Icon, { type: "right" }),
                                        "Next"),
                                    store.currentPage.get() > 0 && store.numPages > 1 && React.createElement(Button, { type: "primary", className: "action-button", onClick: () => store.prevPage() },
