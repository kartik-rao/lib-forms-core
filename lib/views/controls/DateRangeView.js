...
export const DateRangeView = (props) => {
    let component = props.field.componentProps;
    let { dateFormat, mode, defaultStartValue, defaultEndValue } = props.field.componentProps;
    const localStore = useLocalStore(() => {
        return {
            get dates() { return [localStore.defaultStart, localStore.defaultEnd]; },
            defaultValue: component.defaultValue,
            dateMode: (mode ? mode : 'date'),
    ...
    return useObserver(() => {
        return React.createElement(React.Suspense, { fallback: "" },
            React.createElement("span", { id: `${props.field.id}-start`, className: "fl-daterange-field-start", style: { marginRight: '5px' } },
                React.createElement(DatePicker.RangePicker, { onChange: _onChange, format: localStore.dateFormat })));
    });
};
