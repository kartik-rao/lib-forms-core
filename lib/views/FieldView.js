...
let FieldView = class FieldView extends React.Component {
    ...
    render() {
        ...
        return React.createElement("div", { id: field.id, "data-uuid": field.uuid, className: `fl-field fl-${field.inputType}-${field.type}-field` }, !field.isDisabled && React.createElement(Form.Item, Object.assign({ label: field.label, hasFeedback: store.touched[id] && store.errors[id] ? true : null, validateStatus: store.touched[id] && store.errors[id] ? "error" : "validating", id: id }, formItemLayout, { help: store.touched[id] ? (store.errors[id] ? store.errors[id] : field.helpText) : field.helpText, required: field.isRequired }),
