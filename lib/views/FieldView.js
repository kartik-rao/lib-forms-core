...
let FieldView = class FieldView extends React.Component {
    ...
    render() {
        ...
        return React.createElement("div", { id: `fl-field-${field.id}`, "data-uuid": field.uuid, className: `fl-field fl-field-${fieldClass}` }, !field.isDisabled && React.createElement(Form.Item, { label: field.label, hasFeedback: store.touched[id] && store.errors[id] ? true : null, validateStatus: store.touched[id] && store.errors[id] ? "error" : "validating", wrapperCol: itemLayout.wrapperCol, labelCol: layout == "horizontal" ? itemLayout.labelCol : null, extra: field.helpText, help: store.touched[id] ? (store.errors[id] ? store.errors[id] : field.helpText) : '', required: field.isRequired },
