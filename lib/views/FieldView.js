...
export const FieldView = (props) => {
    ...
    let itemLayout = props.field.itemLayoutOptions || fStore.form.itemLayoutOptions || {};
    return useObserver(() => {
        return React.createElement(React.Suspense, { fallback: "loading..." },
            React.createElement("div", { id: `fl-field-${props.field.id}`, "data-uuid": props.field.uuid, className: `fl-field fl-field-${fieldClass}` }, !props.field.isDisabled && React.createElement(Form.Item, { label: props.field.label, labelAlign: labelAlign, hasFeedback: fStore.touched[id] && fStore.errors[id] ? true : null, validateStatus: fStore.touched[id] && fStore.errors[id] ? "error" : "validating", wrapperCol: itemLayout.wrapperCol, labelCol: fStore.form.layout == "horizontal" ? itemLayout.labelCol : null, 
                // extra={props.field.helpText}
                help: fStore.touched[id] ? (fStore.errors[id] ? fStore.errors[id] : props.field.helpText) : '', required: props.field.isRequired },
